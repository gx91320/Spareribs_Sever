    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/product/userstatistics/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userId = data.userStatistics.userId
                this.dataForm.consumeAmount = data.userStatistics.consumeAmount
                this.dataForm.couponAmount = data.userStatistics.couponAmount
                this.dataForm.orderCount = data.userStatistics.orderCount
                this.dataForm.couponCount = data.userStatistics.couponCount
                this.dataForm.commentCount = data.userStatistics.commentCount
                this.dataForm.returnOrderCount = data.userStatistics.returnOrderCount
                this.dataForm.loginCount = data.userStatistics.loginCount
                this.dataForm.attendCount = data.userStatistics.attendCount
                this.dataForm.fansCount = data.userStatistics.fansCount
                this.dataForm.collectProductCount = data.userStatistics.collectProductCount
                this.dataForm.collectSubjectCount = data.userStatistics.collectSubjectCount
                this.dataForm.collectCommentCount = data.userStatistics.collectCommentCount
                this.dataForm.inviteFriendCount = data.userStatistics.inviteFriendCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/userstatistics/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
