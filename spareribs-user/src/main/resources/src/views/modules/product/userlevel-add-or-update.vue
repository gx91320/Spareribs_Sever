<template>
  <el-dialog
    ...
    <el-form-item label="默认等级" prop="defaultStatus">
      <el-input v-model="dataForm.defaultStatus" placeholder="默认等级"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          defaultStatus: [
            { required: true, message: '默认等级不能为空', trigger: 'blur' }
          ],
          freeFreightPoint: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/product/userlevel/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.userLevel.name
                this.dataForm.growthPoint = data.userLevel.growthPoint
                this.dataForm.defaultStatus = data.userLevel.defaultStatus
                this.dataForm.freeFreightPoint = data.userLevel.freeFreightPoint
                this.dataForm.commentGrowthPoint = data.userLevel.commentGrowthPoint
                this.dataForm.priviledgeFreeFreight = data.userLevel.priviledgeFreeFreight
                this.dataForm.priviledgeMemberPrice = data.userLevel.priviledgeMemberPrice
                this.dataForm.priviledgeBirthday = data.userLevel.priviledgeBirthday
                this.dataForm.remark = data.userLevel.remark
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/userlevel/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
